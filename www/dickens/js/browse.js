...
function getIndexList(){
	var url = '/dickens/' + '?operation=browse&mode=index';
	//alert(url)
...
function populateStatsTable(){
	var url = '/dickens/' + '?operation=browse&mode=statstable';
	//alert(url)
	new Ajax.Request(url, {method: 'get', asynchronous: false, 		
	    onSuccess: function(transport) { 
	    	var response = transport.responseText;
	    	var table = response.substring(5,response.indexOf('</xml>'));
		var select = ($('statsTable'))
		select.innerHTML = table;
	    }	    
	 });	
...
function createLiveGrid(url,tabID){
	...
	var grid_opts = {}//{ columnSpecs: [ {width:50}, {width:80} ] };
  	var gridCode = "orderGrid" + tabID + " = new Rico.LiveGrid ('browse_grid_"+tabID+"', new Rico.Buffer.AjaxSQL('" + url + "', params, opts), grid_opts);" 
	eval(gridCode);		
} 
...
function tfpFor(word, indexName){
    ...
    var url = '/dickens/?operation=browse&mode=tfp&index=' + indexName + '&word=' + word;
...
function addIndexTab(indexes) {
	...
	var tabHTML = '<div style="left: 1px; width: 900px; height: 196px;"><p class="ricoBookmark"><span id="browse_grid_' + tabCount + '_bookmark">&nbsp;</span></p>' +
	...
	if (indexes[0].indexOf('gram') == -1 && tfp == true){
		tabHTML += '<th>TFP</th>';
	}
	...
	for (i=0; i<indexes.length; i++) {
		if (indexes[i] == "Raw Freq. from Data") { 
			...
			tabHTML += '<th>' + indexes[i].substring(0, (indexes[i].length)-4)  + ' freq. per ' + normalizeBase + ' words</th>';
		}	
	}	
