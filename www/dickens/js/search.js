...
function doSort(side, number){
	...
	var url = '/dickens/?' + 'operation=search&mode=sort&id=' + rsid + '&side=' + side + '&wordNumber=' + number;
...
function exportKwic(rsid){
    
    var pars = '?operation=search&mode=exportkwic&rsid='+rsid;
    var url = '/dickens/' + pars;
	...
	//alert('In showGrid with ' + tabID + ' ' + rsid);
    	...
    	var tabHTML = '<div><p class="ricoBookmark"><span id="kwic_grid_' + rsid + '_bookmark">&nbsp;</span></p>' +
    			  ...
    			  ' <colgroup><col style="width:40px;" ><col style="width:300px; color: red;" ><col style="width:80px;" ><col style="width:300px;" ></colgroup> ' +
	  		      ...
	  		      '</tr></table><button onclick="exportKwic(\'' + rsid + '\');">Export</button></div>';
...
function getCFP(term){
    ...
    var url = '/dickens/?operation=search&mode=cfp&term=' + term;
...
function buildConcordance(tabID, rsid){
	...
	var url = '/dickens/?operation=search&mode=concordance&id=' + rsid;	
...
// ============== image browsing - probably should be in a separate APU specific script file
function changeImage(direction) {
        image = $('transcriptImage');
        src = image.src;
        src1 = src.substring(0,src.length-6);
        num = parseInt(src.substr(src.length-6,1));
        if (direction=='previous' && num>1) {
                num=num-1;
        } else if (direction=='next' || num < 3) {
                num=num+1;
        }

        newsrc = src1 + num + '.jpeg';

        $('imgcnt').innerHTML = ''+num;
