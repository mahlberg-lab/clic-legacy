...
function showArmTable(tabID, id) {
	...
	var div = $('tab' + tabID);
	div.innerHTML = '<span>Loading: <img src="/dickens/js/images/loading.gif"/></span>';
...
function filterLines(tids) {
	...
	$(tab).innerHTML='<span>Loading: <img src="/dickens/js/images/loading.gif"/></span>';
	var url = '/dickens/?' + 'operation=search&mode=filter&id=' + rsid + '&matchlist=' + tids;
	new Ajax.Request(url, {method: 'get', asynchronous: true, onSuccess: function(transport) { 
		var response = transport.responseText;
